...
public:
  ...
  public:
    ...
    std::size_t _min_size=0;

	for (child_vector::iterator it=children.begin();
	     it!=children.end();
	     ++it) {
      std::size_t child_limit;
	  if (_min_size!=0) {
	    if (limit > 0) {
  ...
  public:
	...
	std::size_t rabin_est = rabin->guestimate();
	std::size_t streett_est = streett->guestimate();
  ...
  public:
    ...
    virtual void calculate(int level, std::size_t limit) {
      if (_options.verbose_scheduler) {
	std::cerr << "Calculate ("<< level <<"): " << typeid(*this).name() << std::endl;
  ...
  public:
	...
	right_stutter_info->checkLTL(_right);
      ...
      if (_options.opt_safra.partial_stutter_check) {
...
std::size_t LTL2DSTAR_Scheduler::calcLimit(std::size_t limit) {
  if (limit==0) {return limit;}
  if (flagOptLimits()) {
    double new_limit=((double)limit*_alpha)+1.0;
    if (new_limit > (double)(std::numeric_limits<std::size_t>::max)()) {
      limit = 0;
    } else {
      limit=(std::size_t)new_limit;
    }
  }
