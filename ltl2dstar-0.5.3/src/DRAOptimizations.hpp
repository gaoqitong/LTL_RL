...
public:
  /** type of a color */
  typedef std::size_t color_t;
  /** type of vector state indizes */
  typedef std::vector<std::size_t> state_vector;
...
private:
  ...
  private:
     ...
     */
    std::vector<std::size_t> _color2state;
  public:
    /** 
     ...
     * @param detailed Keep detailed information on the equivalence classes?
     */
    Coloring(std::size_t size, bool detailed=false) 
      : _nr_of_colors(0), _detailed(detailed) {
    ...
    std::size_t size() const {return _coloring.size();}
    ...
    void setColor(std::size_t state,
		  color_t color) {
    ...
    color_t state2color(std::size_t state) const {
     ...
     * specified color. 
     */
    std::size_t color2state(color_t color) const {
      assert(color<_nr_of_colors);
  ...
  public:
     ...
     * equality.
     */
    bool operator()(std::size_t state_x, 
		    std::size_t state_y) {
  ...
  public:
     ...
     */
    acceptance_signature_t get(std::size_t i) {
  ...
  public:
     ...
     * acceptance signature.
     */
    bool operator()(const std::size_t x,
		    const std::size_t y) {
...
public:
  ...
  optimizeBisimulation(DRA &dra, 
    ...
    for (std::size_t i=0;i<dra.size();i++) {
      states[i]=i;
    }
    ...
    std::size_t old_size=dra.size();
    std::size_t initial_partition=coloring->countColors();

    std::size_t oldColors;
    do {
    ...
    std::size_t new_size=dra_new->size();
...
private:
  ...
  generateDRAfromColoring(DRA &oldDRA,
    ...
    newDRA->setStutterSensitivenessInformation(oldDRA.getStutterSensitivenessInformation());
    ...
    std::size_t old_start_state=oldDRA.getStartState()->getName();
    std::size_t start_state_color=coloring.state2color(old_start_state);
    ...
    for (std::size_t color=0;
	 ...
	 ++color) {
      ...
      std::size_t old_state_representative=coloring.color2state(color);
	...
	if (old_states.cardinality()==1) {
      ...
      std::size_t old_state_index=old_state->getName();
      for (std::size_t i=0;
    ...
    color_t to_color=coloring.state2color(old_to->getName());
