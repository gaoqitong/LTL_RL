 ...
 */
_BITSET_INLINE std::size_t BitSet::cardinality() const {
  std::size_t c=0;
...
BitSet::length() const {
  for (index_t i=storage_size; i>0; i--) {
    if (storage[i-1]!=0) {
      for (index_t j=bits_per_storage; j>0; j--) {
...
BitSet::nextClearBit(index_t fromIndex) const {
  ...
  while (storage_index<storage_size) {
    ...
    tmp=~tmp;
    index_or_nil_t i=getLowestSetBit(tmp);
    if (!i.isNIL()) {
 ...
 * @return the index of the next set bit (or nil if there is no set bit).
 */
_BITSET_INLINE BitSet::index_or_nil_t 
...
_BITSET_INLINE void BitSet::set(index_t bitIndex, bool value) {
  ...
  if (value) {
    storage[storage_index]|=(1 << index_inside_storage);
  } else {
    storage[storage_index]&=~(1 << index_inside_storage);
  }
}
...
_BITSET_INLINE void BitSet::grow(index_t maxIndex) {
  ...
  if (maxIndex >= bits_per_storage*storage_size) {
    // We need to grow it...
    index_t new_size=maxIndex/bits_per_storage+1;
    storage_t *new_storage=new storage_t[new_size];
    for (index_t i=0;i<storage_size;i++) {
      new_storage[i]=storage[i];
    }

    for (index_t j=storage_size;j<new_size;j++) {
      new_storage[j]=0;
    }
 ...
 * @return the index of the highest set bit (or NIL if there are none).
 */
_BITSET_INLINE BitSet::index_or_nil_t 
BitSet::getHighestSetBit(storage_t s) const {
  if (s==0) {return index_or_nil_t::NIL();} 
  
  index_t r_i=0, size=bits_per_storage/2;
  storage_t tmp;
  while (size) {
    tmp=s & (((1<<size)-1) << size);
    if (tmp!=0) {
 ...
 * @param s the storage unit
 * @return the index of the lowest set bit (or NIL if there are none).
 */
_BITSET_INLINE BitSet::index_or_nil_t 
...
_BITSET_INLINE std::ostream& operator<<(std::ostream& out, const BitSet& bs) {
  ...
  bool first=true;
  BitSet::index_or_nil_t i=bs.nextSetBit(0);
  while (!i.isNIL()) {
    if (!first) {
      out << ",";
    } else {
