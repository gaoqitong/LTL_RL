...
class BitSet {
 public:
  ...
  typedef or_nil<index_t> index_or_nil_t;
  ...
  index_or_nil_t nextSetBit(index_t fromIndex) const;
...
public:
  index_or_nil_t getHighestSetBit(storage_t s) const;
  index_or_nil_t getLowestSetBit(storage_t s) const;
};
