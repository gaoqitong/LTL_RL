...
public:
  ...
  virtual void nba_i_switchAPSet(APSet_cp apset) = 0;

  /** 
   * Create a new state.
   * @return the index of the new state
   */
  virtual std::size_t nba_i_newState() = 0;
   ...
   */
  virtual void nba_i_setFinal(std::size_t state, bool final) = 0;
  
  /**
   * Set the state as the start state.
   * @param state the state index
   */
  virtual void nba_i_setStartState(std::size_t state) = 0;
};
