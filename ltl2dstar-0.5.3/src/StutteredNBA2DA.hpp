...
public:
  ...
  private:
	...
	*prefix_signature=*cycle_signature;
	...
	for (std::size_t j=0;j<prefix_signature->getSize();j++) {
    ...
    calculate_prefix_and_cycle_state(intermediate_state_vector_t& state_vector,
      ...
      std::size_t smallest=cycle_point;
      for (std::size_t i=cycle_point+1;i<states;i++) {
	...
	for (std::size_t i=cycle_point; i<state_vector.size();i++) {	  
	...
	PtrComparator<algo_state_t> > intermediate_state_map_t;
      ...
      #ifdef STUTTERED_VERBOSE
      std::cerr << "Calculate from state [" << da_from->getName() << "], "<< (std::size_t) elem << ":" << std::endl;
	...
	if (it==state_map.end()) {
	  ...
	  // found the cycle!
	  std::size_t cycle_point=(*it).second;
