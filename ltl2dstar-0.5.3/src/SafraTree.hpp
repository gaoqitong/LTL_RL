...
public:
  ...
  SafraTreeNode* newNode(std::size_t id) {
    assert(id<MAX_NODES);
    assert(_nodes[id]==0);
   ...
   */
  void removeAllChildren(std::size_t id) {
    assert(id<MAX_NODES);
  ...
  void walkTreePostOrder(SafraTreeVisitor& visitor) {
    std::unique_ptr<SafraTreeWalker<SafraTreeVisitor> >
  ...
  std::size_t getNodeMax() const {return MAX_NODES;}

  /** Get SafraTreeNode with index <i>i</i>*/
  SafraTreeNode* operator[](std::size_t i) {
    return _nodes[i];
  }

  const SafraTreeNode* operator[](std::size_t i) const {
    return _nodes[i];
  }
  ...
  void generateAcceptance(RabinAcceptance::AcceptanceForState acceptance) const {
    for (std::size_t i=0;i<getNodeMax();i++) {
