...
public:
  ...
  StutterSensitivenessInformation() :
    ...
    _hasCheckedHOAProperties(false),
    _hasCheckedLTL(false),
    _hasCheckedNBAs(false) {}

  void checkHOAProperties(const std::vector<std::string>& hoa_properties) {
    if (hasTimekeeping()) {getTimeKeeper().start();}

    for (std::vector<std::string>::const_iterator it = hoa_properties.begin();
	 it != hoa_properties.end();
	 ++it) {
      const std::string& property = *it;

      if (property == "stutter-insensitive") {
	_completelyStutterInsensitive=true;
	_partiallyStutterInsensitive=true;
      }
    }

    _hasCheckedHOAProperties=true;
  ...
  void checkNBAs(NBA_t& nba,
    ...
    if (_printInfo && all_insensitive) {
      // std::cerr << "all insensitive" << std::endl;
    }
