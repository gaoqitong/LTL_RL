...
public:
  ...
  dra2nba(DRA& dra) {
    ...
    nba_p->setStutterSensitivenessInformation(dra.getStutterSensitivenessInformation());
...
private:
  ...
  static void convert_dra2nba(DRA& dra, NBA& nba, bool detailed_states=false) {
    ...
    std::size_t dra_to_index=dra.getIndexForState(edge.second);
  ...
  struct DSABuchiState {
    ...
    /** Constructor */
    DSABuchiState(std::size_t dsa_state_index) :
