...
public:
  ...
  std::size_t add(Indexable *obj);
  void remove(std::size_t index);
  std::size_t size() const;
  Indexable *get(std::size_t index);
  std::size_t get_index(const Indexable *obj) const;

  /** []-operator, get the object at index */
  Indexable *operator[](std::size_t index) {return get(index);}
  ...
  public:
    ...
    Indexable *operator()(std::size_t i) const { return _index->get(i); };
  private:
  ...
  std::pair<bool, std::vector<std::size_t> > compact();
 ...
 */
template <typename Indexable>
void Index<Indexable>::remove(std::size_t index) {
  if (index>=size()) {
 ...
 */
template <typename Indexable>
std::size_t Index<Indexable>::size() const {
  return storage.size();
}
 ...
 */
template <typename Indexable>
Indexable *Index<Indexable>::get(std::size_t index) {
  if (index>=size()) {
    THROW_EXCEPTION(IndexOutOfBoundsException, "");
  }
 ...
 */
template <typename Indexable>
std::size_t Index<Indexable>::get_index(const Indexable *obj) const {
 ...
 */
template <typename Indexable>
std::pair<bool, std::vector<std::size_t> > Index<Indexable>::compact() {
  ...
  // return the mapping
  std::pair<bool, std::vector<std::size_t> > result(moved, mapping);
