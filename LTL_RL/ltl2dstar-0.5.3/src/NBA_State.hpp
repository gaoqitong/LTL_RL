...
class NBA_State : public Indexable<NBA_State<Label,EdgeContainer> > {
  ...
  */
  ...
  void setDescription(const std::string& s) {_description.reset(new std::string(s));}
  /** Get the description for this state. */
  const std::string& getDescription() {assert(_description.get()); return *_description;}

  /** Check if this state has a description. */
  bool hasDescription() {return _description.get();}
...
private:
  ...
  /** A description. */
  std::unique_ptr<std::string> _description;
