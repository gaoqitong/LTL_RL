...
public:
  ...
  /** Returns true if the NBA is deterministic, i.e., if there is either 0 or 1
   * successor state for all edges
   */
  bool isDeterministic() {
    for (std::size_t i=0;
	 i<_nba.size();
	 i++) {
      auto *nba_state=_nba[i];

      for (auto it=nba_state->edges_begin();
	   it!=nba_state->edges_end();
	   ++it) {
	BitSet *to=it->second;
	...
	std::size_t to_cardinality=to->cardinality();
	if (to_cardinality > 1) {
	  return false;
	}
      }
...
private:
  ...
  void calculateStatesWithAllSuccAccepting() {
    ...
    for (std::size_t i=0;i<scc_all_final.size();i++) {
      scc_all_final[i]=false;
    }

    for (std::size_t i=sccs.countSCCs();
	 i>0;
	 --i) {
      // go backward in topological order...
      std::size_t scc=(sccs.topologicalOrder())[i-1];
	...
	if (states_in_scc.length()==1) {
	  // there is only one state in this scc ...
      std::size_t state=states_in_scc.nextSetBit(0);
