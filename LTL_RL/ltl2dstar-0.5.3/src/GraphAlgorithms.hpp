...
public:
  ...
  BitSet const& operator[](std::size_t scc_index) const {
    return _sccs[scc_index];
  }
  
  /** Get the number of SCCs */
  std::size_t countSCCs() const {return _sccs.size();}
  
  /** Get the SCC index for state */
  std::size_t state2scc(std::size_t state) {
    return _state_to_scc[state];
  }

  /** Get a vector with a topological order of the states*/
  std::vector<std::size_t> const& topologicalOrder() {
    return _topological_order;
  }
  
  /** Get a set of SCCs that are successors of the SCC scc_index */
  BitSet& successors(std::size_t scc_index) {
    return _dag[scc_index];
  }
  
  /** Return true, if state_to is reachable from state_from */
  bool stateIsReachable(std::size_t state_from,
		       std::size_t state_to) {
    return isReachable(state2scc(state_from),
		       state2scc(state_to));
  }

  /** Return true, if SCC scc_to is reachable from SCC_from */
  bool isReachable(std::size_t scc_from,
		   std::size_t scc_to) {
    return _reachability[scc_from].get(scc_to);
  }

  /**
   * Return true if the SCC is a bottom SCC, i.e., if all outgoing
   * edges remain in the SCC
   */
  bool isBottom(std::size_t scc_index) {
    return successors(scc_index).isEmpty();
  }
  ...
  friend std::ostream& operator<<(std::ostream& out,
    ...
    for (std::size_t i=0;i<scc.countSCCs();i++) {
      std::size_t scc_i=scc._topological_order[i];
  ...
  std::vector<std::size_t> _state_to_scc;
  std::vector<BitSet> _dag;
  std::vector<std::size_t> _topological_order;
  std::vector<BitSet> _reachability;

  //  friend class GraphAlgorithms::SCC_DFS;
  
  /** Add a new SCC */
  std::size_t addSCC(BitSet& scc) {
    _sccs.push_back(scc);
    return _sccs.size()-1;
  }
  
  /** Set the SCC for a state */
  void setState2SCC(std::size_t state, std::size_t scc) {
...
class StdSuccessorAccess {
 public:
  ...
  successor_iterator begin(Graph& graph, std::size_t v) {
    return graph[v]->successors_begin();
  }

  successor_iterator end(Graph& graph, std::size_t v) {
...
private:
  ...
  private:
	...
	return;
      ...
      if (!disjoint) {
	std::size_t start_idx=start_state->getName();
	visit(start_idx);
      } else {
	...
	if (sdd_w->inComponent==false) {
	  std::size_t dfs_nr_root_v=_dfs_data[sdd->root_index]->dfs_nr;
	  std::size_t dfs_nr_root_w=_dfs_data[sdd_w->root_index]->dfs_nr;
	...
	for (BitSetIterator it=BitSetIterator(states_in_scc);
      ...
      std::vector<std::size_t> sort(_result.countSCCs());
      while (progress) {
	progress=false;

	for (std::size_t scc=0;scc<_result.countSCCs();++scc) {
	  if (in_degree[scc]==0) {
	    sort[scc]=cnt++;
	    progress=true;
	    in_degree[scc]=-1;
	    ...
	    for (BitSetIterator it_neighbors=
		 ...
		 ++it_neighbors) {
	      std::size_t scc_to=*it_neighbors;
	      in_degree[scc_to]--;
	    }
	  }
	}
      ...
      for (std::size_t i=0;i<_result.countSCCs();i++) {
	_result._topological_order[sort[i]]=i;
      }


      // traverse SCCs in reverse topological order
      for (std::size_t i=_result.countSCCs();
	   i>0;
	   --i) {
	std::size_t cur_scc=_result._topological_order[i-1];
